...
function procesarDato(dato: unknown) {
  if (typeof dato === "string") console.log("Es un string:", dato);
...
function convertirJSON<T>(dato: string): T | null {
  try {
    if (!dato.trim()) throw new Error("Debe ser una cadena JSON válida");
    let obj: T = JSON.parse(dato);
    console.log(obj);
    return obj;
  } catch (error) {
    if (error instanceof Error) {
